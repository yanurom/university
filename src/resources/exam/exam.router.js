...
router.route('/:examId').get(async (req, res) => {
...
router.route('/:examId/teachers').get(async (req, res) => {
    const { examId } = req.params;
    const exam = await examService.getById(examId);
...
router.route('/:examId').put(async (req, res) => {
    const { examId } = req.params;
    const { abiturientId, teacherId, subject, date, score } = req.body;
...
router.route('/:examId').delete(async (req, res) => {
    const { examId } = req.params;
